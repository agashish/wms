            ...
            <div class="col-lg-6 warehouseDetails">
              <div class="panel">
				...
				<div class="panel rackdetail">
                                ...
                                <div class="panel-body">
									<div class="row rack_block" style="display:block;" >
										  <?php
										   
										   $inc=0;$i=1; foreach($this->request->data['WarehouseRackData'] as $data)
											{
												//echo $data->$inc->sectionCounter .'==' . $data->Rack->rack_name; exit;
										  ?>
										  
											<div class="rackContainer">
											<div class="col-lg-3">
												<div class="rackBox">
													<a class="add_rack btn btn-labeled btn-success panel-tile margin-bottom-10"><span class="btn-label"><i class="fa fa-eye"></i></span><?php echo $data->Rack->rack_name; ?></a>
													<span class="label bg-red-500 border-grey-100 label-rounded"><?php echo $data->$inc->sectionCounter; ?></span>
												</div>												
											</div>
											</div>
											<?php $i++; } ?>  
...
<script>
	...
	$(document).ready(function(){
		...
		$("body").on("click", ".add_rack", function()
		{
			var getTextOfRack = $(this).text();
			//Check If input rack name exists in table or not
			$.ajax({
					url     	: getUrl() + '/cronjobs/getRackdetailForWarehouseOnClick',
					type    	: 'POST',
					data    	: {
									rackInputName : getTextOfRack											
								  },  			  
					success 	:	function( data  )
					{
						$( 'div.showPopup' ).html( data );
					}                
				});
		});
		...
		$( ".add_fieldset" ).click(function()
				{	
					...
					var url = '<?php echo Router::url('/', true); ?>cronjobs/openRack/'+currentlink;
