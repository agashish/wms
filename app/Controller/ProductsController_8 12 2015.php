...
class ProductsController extends AppController
{
   ...
   public function cat_all_list($p_cid=0 , $innerCounter = 0 ,  $st = 0)
	...
	public function getBarcodeSearch()
	{
		...
		//Ground
		$floorRacks = $this->Rack->find('list' , array( 'fields' => array( 'Rack.floor_name' ) , 'order' => array( 'Rack.id ASC' ) , 'group' => array( 'Rack.floor_name' ) ) );
		$strGround = '<select id="groundFloor" class="form-control" name="data[groundFloor]" multiple>';
		$strGround .= '<option value="">Choose Floor Number</option>';
		foreach( $floorRacks as $rackIndex => $rackValue ):
		...
		//Racks
		$rack = $rackRacks = $rackNameAccordingToFloorText = $this->Rack->find( 'list' , array( 'fields' => array( 'Rack.id' , 'Rack.rack_floorName' ) , 'conditions' => array( 'Rack.floor_name' => $floorRacks ) , 'group' => array( 'Rack.rack_floorName' ) , 'order' => array( 'Rack.id ASC' ) ) );		
		$strRack = '<select id="rackNumber" class="form-control" name="data[rackNumber]" multiple>';
		...
		$strRackLevel = '<select id="levelNumber" class="form-control" name="data[levelNumber]" multiple>';
		$strRackLevel .= '<option value="">Choose Level Number</option>';
		foreach( $rackLevel as $rackIndex => $rackValue ):
		...
		$strRackSection = '<select id="sectionNumber" class="form-control" name="data[sectionNumber]" multiple>';
		$strRackSection .= '<option value="">Choose Section Number</option>';
		foreach( $rackSection as $rackIndex => $rackValue ):
	...
	public function updatePackagingIdByBarcodeSearch()
	{
		...
		$binCombinedText = $this->request->data['groundText'] .'##'. $this->request->data['rackText'] .'##'. $this->request->data['levelText'] .'##'. $this->request->data['sectionText'];
