...
class WarehousesController extends AppController
{
    ...
    var $helpers = array('Html','Form','Common','Session');
     ...
     public function addwarehouse1( $id = null )
    {
    ...
    public function editwarehouse( $id = null )
    {
        ...
        $getRackDataFromDifferentTable = json_decode(json_encode($this->Rack->find('all' , array( 'fields' => array( 'count( Rack.level_association ) as sectionCounter' , 'Rack.rack_name') , 'group' => array('Rack.rack_name'), 'order' => array( 'Rack.level_association ASC' ) ))),0);			   
        
        $getWarehouseRackArrayId = $this->WarehouseRack->find( 'list',array('fields' => array('id'), 'conditions' => array('Warehouse_id'=> $id)) );  
        $getWarehouseArray['WarehouseRackData'] = $getRackDataFromDifferentTable;
