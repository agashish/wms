...
class ProductsController extends AppController
{
    ...
    var $helpers = array('Html','Form','Session','Common' , 'Soap');
   ...
   public function cat_all_list($p_cid=0 , $innerCounter = 0 ,  $st = 0)
	...
	public function uploads_csv()
	{
		...
		if($this->data['Product']['Import_file']['name'] != '')
			...
			for($i=2;$i<=$lastRow;$i++) 
			{
				...
				$productprice['product_price']			=	$objWorksheet->getCellByColumnAndRow(14,$i)->getValue();
				...
				$productdscdata['product_identifier']		=	$objWorksheet->getCellByColumnAndRow(33,$i)->getValue();
				$productdata['minimum_stock_level']				=	$objWorksheet->getCellByColumnAndRow(34,$i)->getValue();
				$productdata['current_stock_level']				=	$objWorksheet->getCellByColumnAndRow(35,$i)->getValue();
				...
				if(count($checkproduct) == 0)
				{
						...
						/* get the id from product and insert the data in product description table */
						$productdscdata['product_id'] = $productid;
						$this->ProductDesc->saveAll($productdscdata, array('validate' => false));
						
						/* get the id from product and insert the data in product price table */
						$productprice['product_id'] = $productid;
						$this->ProductPrice->saveAll($productprice, array('validate' => false));
						
						/* get the id from product and insert the data in productimage table */
	...
	{	 
		  ...
		  $objPHPExcel->getActiveSheet()->setCellValue('F1', 'Previous Stock');		  
		  $objPHPExcel->getActiveSheet()->setCellValue('G1', 'Avb. Stock');
		  $objPHPExcel->getActiveSheet()->setCellValue('H1', 'Min. Stock'); 
		  $objPHPExcel->getActiveSheet()->setCellValue('I1', 'Total Sold'); 
		  ...
		  $inc = 2; foreach( $getStockDetail as $getStockDetailIndex => $getStockDetailValue ):
				...
				$objPHPExcel->getActiveSheet()->setCellValue('F'.$inc, $getStockDetailValue['Product']['uploaded_stock'] );     
				$objPHPExcel->getActiveSheet()->setCellValue('G'.$inc, $getStockDetailValue['Product']['current_stock_level'] );     
				$objPHPExcel->getActiveSheet()->setCellValue('H'.$inc, $getStockDetailValue['Product']['minimum_stock_level'] );     
				
				$objPHPExcel->getActiveSheet()->setCellValue('I'.$inc, ( $getStockDetailValue['Product']['uploaded_stock'] - $getStockDetailValue['Product']['current_stock_level'] ) );     
				
				if( $getStockDetailValue['Product']['product_status'] == 0 ):				
					$objPHPExcel->getActiveSheet()->setCellValue('J'.$inc, 'Active'); 
				else:
					$objPHPExcel->getActiveSheet()->setCellValue('J'.$inc, 'De-Active' ); 
				endif;			  
		  $inc++;
		  endforeach;  
		  
		  //Set First Row for Range Analysis Sheet 
		  $objPHPExcel->getActiveSheet(0)->getStyle('A1:J1')->getAlignment()->applyFromArray(
		  array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
		  'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,));  
		  $objPHPExcel->getActiveSheet(0)->getStyle('A1:J1')->getAlignment()->setWrapText(true);
