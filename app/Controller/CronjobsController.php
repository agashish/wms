...
class CronjobsController extends AppController
{
     ...
     public function beforeFilter()
	{
	    parent::beforeFilter();
	    $this->layout = false;
	    $this->Auth->Allow(array('saveorderdetail', 'assign_services', 'getBarcode', 'setAgainAssignedServiceToAllOrder'));
	}
   ...
   public function createCutOffList()
    ...
    public function updatePackQty()
	{
		...
		$this->loadModel('AttributeOption');
				...
				$objReader= PHPExcel_IOFactory::createReader('CSV');
				$objReader->setReadDataOnly(true);				
				$objPHPExcel=$objReader->load('files/upload _pack_qty.csv');
				$objWorksheet=$objPHPExcel->setActiveSheetIndex(0);
				$lastRow = $objPHPExcel->getActiveSheet()->getHighestRow();
				$colString	=	 $highestColumn = $objPHPExcel->getActiveSheet()->getHighestColumn();
				$colNumber = PHPExcel_Cell::columnIndexFromString($colString);
			...
			for($i=2;$i<=$lastRow;$i++) 
			{
				...
				$this->Product->updateAll(array('Product.pack_qty' => $qty  ), array('Product.product_sku' => $sku));
			}
	}
