<?php
	...
	{	
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL,$time = NULL)
		{
			//print_r($file);
			//echo $destination;
			//die;
			...
			$time = time().'_'.rand(0,123456);
			
			$file['name'] = $time.'.'.$this->ext($file['name']);
			//echo $this->ext($file['name']);die;
				...
				{ 	
					...
					if (is_uploaded_file($fileTmp)) {
						...
						{							
							...
							{
								chmod($output, 0644);
								return $this->result = basename($this->_name);
							} else {
								return $this->error("Could not move '$fileName' to '$destination'");
							}
						}
						else
						{
		...
		{
			$pop	= '<div aria-hidden="false" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="ModalForm" class="modal fade in" style="display: block;"><div class="modal-backdrop fade in" style="height: 700px;"></div>
							  <div class="modal-dialog">
								<div class="modal-content">
								  <div class="modal-header no-border">
									<button data-dismiss="modal" class="close" type="button" onclick="close_popup('."'".$param[2]."'".');"><span aria-hidden="true" >×</span><span class="sr-only">Close</span></button>
								  </div>'.$param[1].
								  '<div class="modal-footer">
										<button type="button" class="btn btn-block btn-success" onclick="close_popup('."'".$param[2]."'".');" >Go To Show User List</button>
								  </div>
								</div>
							  </div>
							</div>';
