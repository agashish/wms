                                            ...
                                            <tr class="odd <?php print $alertClass; ?>">
												...
												<td>
														...
														<?php 	if( $allAttribute['Attribute']['system_generated'] == '0' ) { ?>
															<a href="javascript:void(0);" for="<?php print $id; ?>" class="delete_attribute btn btn-danger btn-xs margin-right-10" title="Retrieved Attribute"><i class="fa fa-close"></i></a>
														<?php } ?>
...
$('body').on('click', '.delete_attribute', function(){
    ...
    function(isConfirm)
    {
        ...
        {
				$.ajax({
							url     : getUrl() + '/Productattributes/deleteattribute',
							type    : 'POST',
							data    : { id : id },
							success : function( msgArray  )
							{
								if(msgArray == 1)
								{
									swal("Please delete Option first.", "" , "error");
								}
								if(msgArray == 2)
								{
									swal("Attribute deleted successfully.", "" , "success");
								}
								location.reload();
							}                
						});
